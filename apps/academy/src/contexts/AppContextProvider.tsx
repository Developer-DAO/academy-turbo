...
export function AppContextProvider({ children }: PropsInterface) {
  ...
  //     const completedIds = completedQuizzesAllData.map((quiz: any) => quiz.lesson); // DEV_NOTE: the -lesson- field now is the lessonId
  ...
  //         (lessonData) => lessonData.projectLessonNumber?.toString() === lesson.slug.toString(), // DEV_NOTE: forcing .toString() to avoid type errors
  //       )?.id;

  //       const completed = currentLessonId ? completedQuizzesIds.includes(currentLessonId) : false; // DEV_NOTE: if the lesson is not found, it is not completed
  ...
  //       const currentLessonId = allLessonsData.find(
  //         (lessonData) => lessonData.fundamentalLessonName?.toString() === lesson.slug.toString(), // DEV_NOTE: forcing .toString() to avoid type errors
  //       )?.id;
  ...
  //   } else if (allLessonsData && fundamentals && completedQuizzesIds.length === 0) {
  //     const fundamentalsWithCompleteStatus = fundamentals.map((lesson) => {
  //       return { ...lesson, completed: false };
  //     });
  //     setFundamentals(fundamentalsWithCompleteStatus);
