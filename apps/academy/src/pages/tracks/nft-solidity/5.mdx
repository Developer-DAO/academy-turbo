...
Lastly add your `NEXT_PUBLIC_CONTRACT_ADDRESS` which is the address of your deployed on Amoy testnet.
...
The Amoy Polygon testnet is newer than the libraries this tutorial is using. Normally you'd search
and replace testnet with `amoy` though for this tutorial, we'll manually add.

Here it the complete `_app.tsx` with changes to use Amoy.
