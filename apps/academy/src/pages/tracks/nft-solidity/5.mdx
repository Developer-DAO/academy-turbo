...
<Callout emoji="💡">Make sure you are in your project's root directory before continuing.</Callout>
...
🚀 Creating a new RainbowKit app in ((path)):/developerdao/frontend
```
...
Create `tiernft.tsx` that will contain the following code:
...
**FIXME: Add a screenshot of the preview**
...
In this application, we have these files:
...
- `useMint.ts` - a custom hook that handles minting
- `useAwaitMintResult.ts` - a custom hook that waits for the minting to complete

### The TierNFT Component

We'll start with the main application logic.

Update `tiernft.tsx` to contain the following code:
...
**FIXME: Add a screenshot of the preview of what the app will look like**
...
Let's just add three of them now and then will explain some common patterns.
...
types for the properties. TypeScript provides meaningful information for your code editor
and provides warnings when you try to pass the wrong type of data to a component.

`SuccessfulMint` component has a range of properties and types:
...
At the end of each file is the CSS in JS styling used by the component. This is a common pattern in React applications.

### Adding Blockchain functionality

So far, we've been building a fairly standard React application in `Next.js` and have configured it
to use `RainbowKit` and `WAGMI` for wallet connections and blockchain interactions.

We haven't yet added any blockchain functionality to our application. We'll do that next.
...
We use the `usePrepareContractWrite` hook to prepare the contract for writing. This hook returns a
`config` object that we can use to mint the NFTs.

We then use the `useContractWrite` hook to write the minting transaction to the blockchain.
This hook returns a `mintData` object that contains the transaction hash and a `mint` function
that we can call to mint the NFTs.
...
When a NFT successfully mints, an event is emitted from the smart contract.
Our `useAwaitMintResult` hook is listening for this event, and when it happens the hook saves the
new tokenId which we need to create a link to the NFT in OpenSea. It also fetches the NFT's metadata
from the contract and saves it (to `latestNFTMinted`). The hook then returns these two values to the
main application to display in the UI.

** --- FIXME: BRIAN LEFT OFF HERE WHILE UPDATING LESSON --- **
...
The [`useContractWrite`](https://1.x.wagmi.sh/react/hooks/useContractWrite) hook from
...
The `useContractWrite` hook also work better with the `usePrepareContractWrite` hook
as explained on [wagmi docs](https://1.x.wagmi.sh/react/hooks/useContractWrite#prepared-usage).
...
- We pass in `e` as an argument so that we can pick the amount of ETH associated
  ...
  [here](https://1.x.wagmi.sh/react/hooks/useContractWrite#configuration).
