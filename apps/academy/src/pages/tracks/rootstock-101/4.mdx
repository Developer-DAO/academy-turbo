...
Now you will add the ABI to the `src/pages/index.tsx` file. This will allow your app to interact with parts needed on the frontend.
...
The first step is using the setState hook in React to setup options and the write contract hook in wagmi. 

Remove any boilerplate code in the `Home` function and add the following code:
...
Since we are using the `useEffect` hook, make sure to import it at the top of the file:
...
If you are not already running your development server, run the `npm run dev` command in the terminal.
