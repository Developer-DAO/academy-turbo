...
Then came Web2, and with it development, a paradigm shift occured, introducing a
...
{/\* - KEMP: Grove has a free tier which you can see on their pricing page, what seems to be breaking the flow here is the process to redeem benefits as it assumes users are already DAO member which in most cases is likely not to be true.

- Suggest updating the language to recommend Grove or Nodies, and then adding a note to say if you're a developer dao member you can get a discount for these services here - and provide a link to the benefits page here.

- Consider breaking up sections a little further by either using bullet points or more sub-headings. There is a lot of text in this block below, might be worth adding section like:
  "What RPC to use" or "Recommended RPCs"
  the bullet point approach on the last version of this lesson here I personally find easier to read. \*/}
...
We're going to look at POKT Network, a decentralised RPC base layer that operates as an ecosystem of
...
If you're a Developer DAO member, you are hopefully aware of some of the "D*D Perks" for Web3 we have.
Let's take advantage of one of those, and get you hooked up with Grove, one of the \_Gateways* built on
top of POKT Network's RPC baselayer. [Click here for Grove](https://devdao.to/grove-perk).

Are you not yet a member of Developer DAO? There is no time like the present 😁. If accessing an RPC
Gateway is a more pressing priority, choose [Nodies, another Gateway from POKT](https://www.nodies.app/)
and easily set up an account for yourself.
