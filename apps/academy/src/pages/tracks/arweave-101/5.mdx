...
<LessonLayout
   ...
   lessonDescription="Deploy a DApp to Arweave."
   author="K"
...
You will create a single-page application (SPA) with [Vite](https://vitejs.dev/), a popular web application bundler. You will use React as a UI framework and handle Arweave uploads with the Turbo SDK in the browser.

## Prerequisites

You must **complete lessons 1, 2, and 3**. You need the **wallet address you charged with Turbo Credits** in lesson 2 and **the ArNS name you registered** in lesson 3.
...
The script logs the URL to the deployment, so you can immediately test it by opening it in your browser. If you want to see what it looks like, try [my deployment](https://arweave.developerdao.com/8jctPVVUkKnvGhzXJpQsqj-E0ZKSOr1XYZlcB4sstqw).
